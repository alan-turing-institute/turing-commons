...
We're going to pretend that we there is a project team developing a classification model, using supervised learning, which will try to predict whether a patient has a disease or not based on a set of features.
...
However, despite not using `sex` when training their model, the model still ended up discriminating against female patients—perhaps due to some other proxy feature (or combination of features) in the model that was highly correlated with `sex`.
...
It is important to note that these criteria capture a particular notion of fairness (i.e. non-discrimination), but that there are other notions of fairness that are not captured by these criteria (see [section 1](rri-203-1.md)).

<!-- begin admonition -->
**Individual vs. Group Fairness**

This section is concerned with group-level fairness criteria, and the introduction of a tool that helps to decide between them.
...
To understand these criteria, we need to define a few key variables:
...
Now, we can define the three criteria formally and intuitively as follows:
...
- Separation:
  ...
  - Intuition: membership in a protected group may be correlated with the target variable, so independence may be unachievable. Instead, separation requires that the proportion of correct predictions are equal across groups, (e.g a male and female who have the disease are equally likely to be correctly predicted as having the disease)
- Sufficiency:
  - Formal: $A$ is *conditionally* independent of the true outcome $Y$, given the model's predictions $\hat{Y}$
  - Intuition: [...]

Each of these criteria represent some objective to be achieved in the development of a model, but their motivations are different and lead to different conceptions of fairness.
...
The Fairness Compass is a tool created by Ruf and Detyniecki (2021) to help data scientists choose between the three fairness criteria we have just discussed.
...
<!-- insert fairness compass tool graphic -->

<!-- insert admonition -->
**Interactive Tool and Paper**
...
In the case of the medical diagnosis example, this would be the true diagnosis of each patient, which we previously referred to as $Y$.
...
In the case of medical diagnosis, this may be achievable through secondary testing.
For instance, if we know that our first diagnostic test is good but not perfect (e.g. it optimises for recall so as not to miss any cases), then we can use a second test that has higher precision to confirm a diagnosis and reduce the number of false positives.
...
None of these questions should be misconstrued as undermining the existence or lived experience of mental health conditions—this is undeniable.
...
Here, 'health care cost' was used in place of the underlying ground truth.
...
[^fairmlbook]: add fairmlbook reference
